...
export function CardFooter({ children }: { children: React.ReactNode }) {
  return (
    <p className="mt-auto pt-4 text-[11px] font-medium text-muted-foreground">{children}</p>
  );
}
...
export function CardError({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-3 py-8 text-center">
      <span className="flex h-10 w-10 items-center justify-center rounded-full bg-rose-500/10 text-rose-600 dark:text-rose-400">
        <AlertCircle className="h-5 w-5" />
      </span>
      <p className="text-xs text-muted-foreground">Couldn&apos;t load this data.</p>
      <button
        type="button"
        onClick={onRetry}
        className="inline-flex items-center gap-1.5 rounded-full border border-border bg-secondary px-3 py-1.5 text-xs font-medium text-foreground transition-colors hover:border-primary/40 hover:text-primary"
      >
